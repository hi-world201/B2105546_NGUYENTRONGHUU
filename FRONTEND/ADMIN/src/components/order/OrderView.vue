<template>
  <div>
    ...
    <div class="container px-5 py-3 page">
      <legend class="form-title">Thông tin đơn hàng</legend>
...
const getNextStatusString = computed(() => {
  if (order.value.currentStatus === 'pending') return 'Xác nhận đơn hàng';
  if (order.value.currentStatus === 'accepted') return 'Giao hàng';
});
